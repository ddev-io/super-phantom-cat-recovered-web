...
var LyGameOver = vee.Class.extend({
  ...
  onCreate: function () {
    ...
    __goCall(game.Data, "addMiniCount");

    var savePointIdx = __goCall(game.Data, "getSavePointIdx");
    this._canRevive = (savePointIdx != null && ("" + savePointIdx) === ("" + game.Data.performingTMXIdx));
  ...
  onLoaded: function () {
    ...
    if (game.Data.isFreeGame && !game.Data.shownVideoAd) {
      __goCall(vee.Ad, "showInterstitialByOnlineConfig", "over");
      if (vee.Analytics && vee.Analytics.UGameEvent) {
        __goCall(vee.Analytics.UGameEvent, "showAdEvent", "over", "interstial");
      }
  ...
  onloadEvent: function () {
    if (game.Data.version && game.Data.version.isKTPlay) {
      ...
      __goCall(this, "playAnimate", "LoseNoAd");
    }
    ...
    if (vee.PopMgr && vee.PopMgr.PositionType) {
      ...
      __goCall(vee.PopMgr, "setNodePos", this.nodeL, vee.PopMgr.PositionType.Left);
    }
    ...
    __goPrepareAvatarProgress(this, LyGameOver);
    __goSetHeroTexture(this);
    __goInitGameOverLabels(this, LyGameOver);

    __goCall(game.LevelData, "save");
    __goLogMissionFailed();
    __goVisible(this.btnKTPlay, !!(game.Data.version && game.Data.version.isKTPlay));
  },

  showBanner: function () {
    __goCall(vee.Ad, "showBannerAd");
  ...
  initController: function () {
    ...
    var avatarStoreCtl = this.ccbAvatarStore && this.ccbAvatarStore.controller;
    var openStore = avatarStoreCtl && avatarStoreCtl.onOpenStore ? avatarStoreCtl.onOpenStore.bind(avatarStoreCtl) : function () {};
    var shopButton = (typeof BtnShop !== "undefined" && BtnShop.instance) ? BtnShop.instance.btnOpenStore : null;

    __goCall(vee.Controller, "registerItemByButton", shopButton, __goPoint(0, 0), openStore, res.mfi_home_avatar_on_png);
    ...
    if (vee.Utils && __goCall(vee.Utils, "isRecordAvaliable") && this.ccbRecord && this.ccbRecord.controller) {
      var recordCtl = this.ccbRecord.controller;
      __goCall(vee.Controller, "registerButtonAction", [vee.KeyCode.AXIS_LEFT_TRIGGER, vee.KeyCode.BUTTON_LEFT_SHOULDER], recordCtl.onTouch.bind(recordCtl));
    }

    if (avatarStoreCtl) {
      __goCall(vee.Controller, "registerButtonAction", [vee.KeyCode.AXIS_RIGHT_TRIGGER, vee.KeyCode.BUTTON_RIGHT_SHOULDER], openStore);
    }
  ...
  onRetry: function () {
    ...
    if (__goCall(game.Data, "costEnergy")) {
      this.loseRetry();
    } else {
      __goCall(game.Data, "onEnergyEmpty", game.LifeEmptyType && game.LifeEmptyType.Lose);
    }

    __goCall(vee.Ad, "hideBannerAd");
    __goCall(vee.Controller, "clearAll");
  ...
  loseRetry: function () {
    ...
    if (typeof EfxCostEnergy !== "undefined" && this.btnRetry) {
      __goCall(EfxCostEnergy, "show", __goCall(this.btnRetry, "getPosition"), this.rootNode);
    }

    var self = this;
    __goCall(vee.Transition, "out", res.MapTransition_ccbi, function () {
      self._openGame(self._canRevive);
    });

    game.Data.retryCount = (game.Data.retryCount || 0) + 1;
  },
  ...
  onAd: function () {
    ...
    if (typeof LyReward !== "undefined" && LyReward.show) {
      LyReward.show();
    } else {
  ...
  showMini: function () {
    if (typeof LySpecial !== "undefined" && LySpecial.show) {
      LySpecial.show();
    } else if (typeof LyGameOverMini !== "undefined" && LyGameOverMini.show) {
      LyGameOverMini.show();
    }
  },
  ...
  checkAvatar: function (coin) {
    ...
    __goCall(game.LevelData, "addCoin", coin);

    var next = this._nextAvatar || (LyGameOver.shouldShowAvatarProgress() ? __goCall(game.AvatarData, "getNextAvatar") : null);
    if (next && next.price) {
      var total = __goCall(game.LevelData, "getCoin") || 0;
      if (total >= next.price) {
        var idx = next.idx;
        if (idx >= 11) {
          idx -= 1;
        }
        this.unlockedIdx = idx;
        this._isUnlock = true;
        __goCall(game.AvatarData, "avatarPurchased", idx);
      }
    }
    __goUpdateVisibleProgress(this, LyGameOver);
  },
  ...
  _progressAnimate: function () {
    ...
    LyGameOver.targetCoin = this._nextAvatar.price || LyGameOver.targetCoin;

    if (this._isUnlock) {
      this._showCoinParticle(Math.max(0, (LyGameOver.targetCoin || 0) - (this._coin || 0)));
    } else {
  ...
  updateShowCoin: function () {
    ...
    if (this._coinOff <= 0 && this.unlockedIdx >= 0) {
      ...
      if (this.oCoinCtl) {
        this.oCoinCtl.setNumber(LyGameOver.targetCoin || 0);
      }
    }
...
LyGameOver.shouldShowAvatarProgress = function () {
  return !!(game.Data && game.Data.stageType === game.StageType.Event);
};

LyGameOver.formatFunc = function (label, value) {
  __goSetString(label, value + " / " + (LyGameOver.targetCoin || 0));
};

LyGameOver.show = function () {
  var layer = __goCall(vee.PopMgr, "popCCB", res.GameOver_ccbi, { alpha: 0 });
  if (layer && layer.controller) {
    __goCall(layer.controller, "ccbInit");
    __goCall(layer.controller, "onLoaded");
  }
  return layer;
};
