...
(function (root) {
	var state = {
		lines: [],
		overlay: null,
		showOverlay: !!root.__supercatShowBootOverlay
	};
...
function __supercatIsAudioResource(item) {
	var src = item;
	if (item && typeof item === "object") {
		src = item.src;
	}
	return typeof src === "string" && /\.(?:mp3|m4a|ogg|wav|mp4)(?:[?#].*)?$/i.test(src);
}

function __supercatIsImageResource(item) {
	var src = item;
	if (item && typeof item === "object") {
		src = item.src;
	}
	return typeof src === "string" && /\.(?:png|jpg|jpeg|gif|bmp|ico|tiff|webp)(?:[?#].*)?$/i.test(src);
}

// TMX tilesets are consumed synchronously by cc.TMXTiledMap.create(), so they
// must be in the texture cache before a level is built; a deferred load leaves
// the map blank ("no texture bound to target"). In this title every tileset is
// named forest*.png, so we keep just those in the startup preload even when the
// bulk image preload is skipped. These 21 tilesets cost about 55 MiB of RGBA.
function __supercatIsTilesetImage(item) {
	var src = item;
	if (item && typeof item === "object") {
		src = item.src;
	}
	...
	return /^forest.*\.(?:png|jpg|jpeg|webp)$/i.test(base);
}
...
// The APK dump keeps res/ almost flat, while CCBI files still reference the
// original CocosBuilder folder layout ("big/...", "game/..."). Remap any path
// whose file is missing but whose basename exists at the res/ root, otherwise
// those sprites silently 404 on case-sensitive/packaged targets like webOS.
...
function __supercatInstallFlatPathFallback() {
	...
	if (cc.textureCache && typeof cc.textureCache.addImage === "function" && !cc.textureCache.__supercatFlatPathWrapped) {
		var __supercatAddImage = cc.textureCache.addImage;
		cc.textureCache.addImage = function (url, cb, target) {
			return __supercatAddImage.call(this, __supercatFixResourcePath(url), cb, target);
		};
	...
	if (cc.spriteFrameCache && typeof cc.spriteFrameCache.addSpriteFrames === "function" && !cc.spriteFrameCache.__supercatFlatPathWrapped) {
		var __supercatAddSpriteFrames = cc.spriteFrameCache.addSpriteFrames;
		cc.spriteFrameCache.addSpriteFrames = function (url, texture) {
			return __supercatAddSpriteFrames.call(this, __supercatFixResourcePath(url), texture);
		};
...
function __supercatPreloadSpriteFrameFiles() {
    if (__supercatBootRoot.__supercatLowMemory && __supercatBootRoot.__supercatMemory) {
        __supercatBootRoot.__supercatMemory.indexAtlases();
        return;
    }
...
function __supercatPatchSpriteFrameCacheLookup() {
	...
	frameCache.getSpriteFrame = function (name) {
		...
		if (!frame && !this.__supercatResolvingMissingFrame) {
            ...
            try {
                if (__supercatBootRoot.__supercatLowMemory && __supercatBootRoot.__supercatMemory)
                    __supercatBootRoot.__supercatMemory.ensureFrame(name);
                else
...
if (typeof cc !== "undefined" && cc.loader && typeof cc.loader._loadResIterator === "function" && !cc.loader.__supercatSafeIterator) {
	...
	cc.loader._loadResIterator = function (item, index, cb) {
		...
		if (typeof item === "string") {
			item = __supercatFixResourcePath(item);
		} else if (item && typeof item.src === "string") {
			item.src = __supercatFixResourcePath(item.src);
		}
...
if (typeof cc !== "undefined" && cc.loader && typeof cc.loader.loadImg === "function" && !cc.loader.__supercatFlatPathWrapped) {
	var __supercatLoadImg = cc.loader.loadImg;
	cc.loader.loadImg = function (url, option, callback) {
		return __supercatLoadImg.call(this, __supercatFixResourcePath(url), option, callback);
	};
	cc.loader.__supercatFlatPathWrapped = true;
}

function __supercatTraceRenderInfo() {
	var root = __supercatBootRoot;
	try {
		var isWebGL = typeof cc !== "undefined" && cc.game && cc._renderType === cc.game.RENDER_TYPE_WEBGL;
		var line = "renderer=" + (isWebGL ? "WebGL" : "Canvas");
		var gl = isWebGL ? cc._renderContext : null;
		if (gl && typeof gl.getParameter === "function") {
			var dbgInfo = gl.getExtension("WEBGL_debug_renderer_info");
			if (dbgInfo) {
				line += " gpu=" + gl.getParameter(dbgInfo.UNMASKED_RENDERER_WEBGL);
			} else {
				line += " gl=" + gl.getParameter(gl.VERSION);
			}
		}
		root.__supercatBootTrace(line);
		root.__supercatBootTrace("ua=" + ((typeof navigator !== "undefined" && navigator.userAgent) || "?"));
		root.__supercatBootTrace("frame=" + ((typeof window !== "undefined" && window.innerWidth + "x" + window.innerHeight) || "?") +
			" dpr=" + ((typeof window !== "undefined" && window.devicePixelRatio) || 1));
	} catch (err) {
		root.__supercatBootTrace("render info error: " + err);
	}
}

function __supercatInstallSlowResourceLoader() {
	var root = __supercatBootRoot;
	var concurrency = parseInt(root.__supercatResourceLoadConcurrency, 10) || 0;
	var delayMs = parseInt(root.__supercatResourceLoadDelayMs, 10) || 0;
	if ((concurrency <= 0 && delayMs <= 0) || !cc || !cc.loader || !cc.AsyncPool || cc.loader.__supercatSlowLoadWrapped) {
		return;
	}
	concurrency = concurrency > 0 ? concurrency : 0;
	delayMs = delayMs > 0 ? delayMs : 0;
	var originalLoad = cc.loader.load;
	cc.loader.load = function () {
		var originalAsyncPool = cc.AsyncPool;
		cc.AsyncPool = function (srcObj, limit, iterator, onEnd, target) {
			var effectiveLimit = concurrency || limit;
			var delayedIterator = iterator;
			if (delayMs > 0) {
				delayedIterator = function (value, index, callback, pool) {
					iterator.call(this, value, index, function () {
			...
			return new originalAsyncPool(srcObj, effectiveLimit, delayedIterator, onEnd, target);
		};
		cc.AsyncPool.prototype = originalAsyncPool.prototype;
		try {
			return originalLoad.apply(this, arguments);
		} finally {
			cc.AsyncPool = originalAsyncPool;
		}
	};
	cc.loader.__supercatSlowLoadWrapped = true;
	root.__supercatBootTrace("resource loader throttled: concurrency=" + concurrency + " delayMs=" + delayMs);
}

function __supercatStartBackgroundTexturePreload() {
	var root = __supercatBootRoot;
	if (root.__supercatDisableBackgroundTexturePreload || root.__supercatBgPreloadStarted || !root.__supercatSkipImageStartupPreload) {
		return;
	}
	if (typeof cc === "undefined" || !cc.textureCache || typeof cc.textureCache.addImage !== "function") {
		return;
	}
	var config = root.__supercatSpriteFrameFileListConfig ||
		(cc.loader && cc.loader.getRes && cc.loader.getRes("res/spriteFrameFileList.plist"));
	var files = config && config.spriteFrameFiles;
	if (!files || !files.length) {
		root.__supercatBootTrace("bg texture preload: atlas list unavailable");
		return;
	}
	root.__supercatBgPreloadStarted = true;
	var paths = [];
	for (var i = 0; i < files.length; i++) {
		paths.push("res/" + String(files[i]).replace(/\.plist$/i, ".png"));
	}
	var idx = 0, loadedCount = 0;
	// Decoding atlases bursts the TV's CPU and tanks the frame rate, so trickle
	// one texture at a time with generous gaps.
	var CHUNK = 1, GAP_MS = 700, BUSY_RETRY_MS = 1500, START_DELAY_MS = 8000;
	function isGameplayActive() {
		var clock = (typeof game !== "undefined" && game.Data) ? game.Data._lastLyGameUpdateClock : 0;
		return !!clock && (Date.now() - clock) < 1200;
	}
	function step() {
		if (idx >= paths.length) {
			root.__supercatBootTrace("bg texture preload done: " + loadedCount + "/" + paths.length);
			return;
		}
		// Stay out of the way while a level is running; resume in menus/pause.
		if (isGameplayActive()) {
			setTimeout(step, BUSY_RETRY_MS);
			return;
		}
		var batch = paths.slice(idx, idx + CHUNK);
		idx += batch.length;
		...
		for (var j = 0; j < batch.length; j++) {
			try {
				cc.textureCache.addImage(batch[j], onLoaded);
			} catch (err) {
				onLoaded();
			}
		}
	}
	root.__supercatBootTrace("bg texture preload scheduled: " + paths.length + " atlases");
	setTimeout(step, START_DELAY_MS);
}

cc.game.onStart = function(){
	__supercatBootRoot.__supercatBootTrace("cc.game.onStart");
	__supercatTraceRenderInfo();
	__supercatInstallSlowResourceLoader();
	__supercatInstallFlatPathFallback();
    if (__supercatBootRoot.__supercatMemory) __supercatBootRoot.__supercatMemory.install();
	__supercatPatchEventManager();
	...
	var allPreloadResources = (g_resources || []).filter(__supercatIsPreloadResource);
	var preloadResources = allPreloadResources;
	if (__supercatBootRoot.__supercatSkipAudioStartupPreload) {
		preloadResources = allPreloadResources.filter(function (item) {
			return !__supercatIsAudioResource(item);
		});
		__supercatBootRoot.__supercatBootTrace("startup audio preload skipped=" + (allPreloadResources.length - preloadResources.length));
	}
	if (__supercatBootRoot.__supercatSkipImageStartupPreload) {
		var beforeImageSkip = preloadResources.length;
		preloadResources = preloadResources.filter(function (item) {
			// Keep TMX tilesets: maps create their texture synchronously.
			return !__supercatIsImageResource(item) || __supercatIsTilesetImage(item);
		});
		__supercatBootRoot.__supercatBootTrace("startup image preload skipped=" + (beforeImageSkip - preloadResources.length));
	}
    ...
    cc.LoaderScene.preload(preloadResources, function () {
		...
		if (typeof main === "function") {
			...
			__supercatStartBackgroundTexturePreload();
	    	cc.gameInited = true;
			return;
		}
